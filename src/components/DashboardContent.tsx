...
const DashboardContent = ({
  ...
  if (isLoading) {
    return <div className="p-6 space-y-6">
        ...
        {[...Array(4)].map((_, i) => <Card key={i} className="card-soft p-6 h-24 animate-pulse"></Card>)}
      </div>;
  }
  ...
  return <motion.div className="p-6 space-y-6" variants={containerVariants} initial="hidden" animate="visible">
      ...
      {/* Training tip */}
      {tip && <motion.div variants={itemVariants}>
      ...
      {mission && <motion.div variants={itemVariants}>
          <Card className="card-soft p-6 bg-orange-100">
            <div className="flex items-start space-x-3">
              ...
              <Checkbox checked={missionCompleted} onCheckedChange={toggleMissionCompleted} className="w-6 h-6" id="daily-mission" />
            </div>
          </Card>
        </motion.div>}
