      ...
      <CardContent className="space-y-6">
        {isBaseline && (
          <Alert variant="default" className="bg-blue-50 border-blue-200">
            ...
            <AlertTitle className="text-blue-700">첫 분석 완료!</AlertTitle>
            <AlertDescription className="text-blue-600">
              이번 분석 결과가 앞���로의 모든 변화를 비교하는 '기준점'으로 저장되었어요.
            </AlertDescription>
          </Alert>
        )}
        <div>
          <h3 className="font-semibold mb-2">분석 결과 영상</h3>
          <video src={processedVideoUrl!} controls className="w-full rounded-lg bg-black">
            브라우저가 비디오 태그를 지원하지 않습니다.
          </video>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
          <Card className="p-4">
            <p className="text-sm text-gray-500">안정성</p>
            <p className="text-2xl font-bold">{analysisResult?.stability}점</p>
          </Card>
          <Card className="p-4">
            <p className="text-sm text-gray-500">좌우 대칭성</p>
            <p className="text-2xl font-bold">{analysisResult?.symmetry}%</p>
          </Card>
          <Card className="p-4">
            <p className="text-sm text-gray-500">평균 보폭</p>
            <p className="text-2xl font-bold">{analysisResult?.stride_length}cm</p>
          </Card>
        </div>
        <div className="flex justify-end">
            <Button onClick={() => navigate('/history/joint-analysis')}>
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 max-w-4xl">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-gray-800">AI 관절 움직임 분석</h1>
        <p className="text-gray-600 mt-2">
          강아지의 걷는 영상을 업로드하여 관절의 움직임을 분석하고, 변화를 추적하여 건강을 관리하세요.
        </p>
      </div>
...
export default JointAnalysisPage;